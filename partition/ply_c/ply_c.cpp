...
{//compute the following geometric features (geof) features of a point cloud:
 //linearity planarity scattering verticality
    ...
    std::vector< std::vector< float > > geof(n_ver, std::vector< float >(5,0));
    ...
    {//each point can be treated in parallell independently
        ...
        float qulv = std::min(std::min(sqrtf(lambda[0]),sqrtf(lambda[1])),std::min(sqrtf(lambda[1]),sqrtf(lambda[2])))/(sqrtf(lambda[0])+sqrtf(lambda[1])+sqrtf(lambda[2]));
...
PyObject * compute_rgb(const bpn::ndarray & rgb ,const bpn::ndarray & target, int k_nn)
{//compute the following geometric features (geof) features of a point cloud:
 //linearity planarity scattering verticality
    std::size_t n_ver = bp::len(rgb);
    std::vector< std::vector< float > > geof(n_ver, std::vector< float >(1,0));
    //--- read numpy array data---
    const uint32_t * target_data = reinterpret_cast<uint32_t*>(target.get_data());
    const float * rgb_data = reinterpret_cast<float*>(rgb.get_data());
    std::size_t s_ver = 0;
    #pragma omp parallel for schedule(static)
    for (std::size_t i_ver = 0; i_ver < n_ver; i_ver++)
    {//each point can be treated in parallell independently
        ...
        std::size_t i_edg = k_nn * i_ver;
        std::size_t ind_nei;
        position(0,0) = rgb_data[3 * i_ver];
        position(0,1) = rgb_data[3 * i_ver + 1];
        position(0,2) = rgb_data[3 * i_ver + 2];
        for (std::size_t i_nei = 0; i_nei < k_nn; i_nei++)
        {
                //add the neighbors to the position matrix
            ind_nei = target_data[i_edg];
            position(i_nei+1,0) = rgb_data[3 * ind_nei];
            position(i_nei+1,1) = rgb_data[3 * ind_nei + 1];
            position(i_nei+1,2) = rgb_data[3 * ind_nei + 2];
            i_edg++;
        }
        // compute the covariance matrix
        ei::MatrixXf centered_position = position.rowwise() - position.colwise().mean();
