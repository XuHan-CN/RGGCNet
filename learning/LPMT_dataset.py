...
def get_datasets(args, test_seed_offset=0):
    ...
    for n in range(1,7):
        if n != args.cvfold:
            path = '{}/superpoint_graphs/LPMT/'.format(args.LPMT_PATH, n)
    ...
    path = '{}/superpoint_graphs/LPMT/'.format(args.LPMT_PATH, args.cvfold)
    ...
    return tnt.dataset.ListDataset([spg.spg_to_igraph(*tlist) for tlist in trainlist],
                                    functools.partial(spg.loader, train=True, args=args, db_path=args.LPMT_PATH)), \
           tnt.dataset.ListDataset([spg.spg_to_igraph(*tlist) for tlist in testlist],
                                    functools.partial(spg.loader, train=False, args=args, db_path=args.LPMT_PATH, test_seed_offset=test_seed_offset)), \
...
def get_info(args):
    ...
    if args.loss_weights == 'none':
        ...
        weights = h5py.File(args.LPMT_PATH + "/parsed/class_count.h5")["class_count"][:].astype('f4')
    ...
    return {
        ...
        'classes': 13,
        'inv_class_map': {0:'ceiling', 1:'floor', 2:'terrain', 3:'column', 4:'beam', 5:'window', 6:'door', 7:'table', 8:'debris', 9:'bookcase', 10:'sofa', 11:'board', 12:'clutter'},
    }
...
def preprocess_pointclouds(args):
    ...
    LPMT_PATH = args.LPMT_PATH
    class_count = np.zeros((13,6),dtype='int')
    for n in range(1,7):
        pathP = '{}/parsed/LPMT/'.format(LPMT_PATH, n)
        if args.supervized_partition:
            pathD = '{}/features_supervision/LPMT/'.format(LPMT_PATH, n)
        else:
            pathD = '{}/features/LPMT/'.format(LPMT_PATH, n)
        pathC = '{}/superpoint_graphs/LPMT/'.format(LPMT_PATH, n)
    ...
    path = '{}/parsed/'.format(LPMT_PATH)
    data_file = h5py.File(path+'class_count.h5', 'w')
    data_file.create_dataset('class_count', data=class_count, dtype='int')

if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description='RGGCNet')
    parser.add_argument('--LPMT_PATH', default='datasets/LPMT')
